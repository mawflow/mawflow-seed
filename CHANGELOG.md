...
- 修复派生项目模板提交与 Seed Contract 同时落后时，升级提示只推进模板基线、遗漏契约迁移的问题；当前会话必须完成受控迁移与最终复检。
- 源模板 `.maw/seed.lock` 缺失、损坏或契约字段不完整时返回 `seed_contract_unavailable`，保留独立模板关系，不再误报已对齐；较新的项目 Seed 继续禁止回退。
- 明确模板计划器以源模板为比较目标、工作台以本机 Seed Kit 为比较目标，两条证据分别验收。

## v2.8.2 - 2026-09-06

- 修复公开目录检查把根 `.git` 误报为嵌套 Git 污染的问题；真实 Git clone 与 worktree 均可通过，组件内的 `.git` 文件、空目录及符号链接仍受检查。
- 增加真实 Git checkout 和嵌套 metadata 回归，保留 v2.8.1 反馈报告能力与 Contract 2 兼容。
...
- 统一共享入口语义、Kit 分发、存量受控迁移和受管区块，保留人工规则。
- 新增独立 Agent 就绪检查与有界上下文导出，补齐空入口、越界、迁移和回滚验证。


## v2.7.0 - 2026-09-04

- 新增 Seed 脚本用途合同，把本地调试、检查验证、配置帮助、发布计划、交付运维和项目扩展脚本分组，供本地/云端工作台复用，不建立第二套 UI 私有分类。
- `npm run dev` 保持规范入口，`local:dev` 按默认应用、前后端、全部可运行或当前设备自定义范围调用组件真实 `dev/start`；新增 `dev:list`、`dev:status` 和 `dev:stop`。
- 当前设备选择与运行记录进入 Git 已忽略的 `.local/.maw/app-runtime.yaml` 和 `.local/run/local-dev/`；未选应用不启动、不探测、不阻塞。
- 本地调试阻塞收敛为选择、真实命令、规范入口和当前范围一次成功后台启动四项；工具链、依赖、编译和发布检查继续作为独立 Seed 规范事实。
- 存量项目通过 T3 增量迁移保留已有业务脚本语义和项目事实，再接入可选择启动器；工作台执行继续使用预览、确认、后台进度、脱敏日志和所属高风险流程。

## v2.6.0 - 2026-09-02

- 新增 `.maw/deployments.yaml`，支持一个线上/生产环境配置多个部署目标，每个目标绑定可复用服务器资产、可选子项目与显式组件列表。
- 生产门禁改按规范 `environment_role` 判断，自定义环境 key 不能绕过审批、健康、备份与回滚要求；组件范围、服务器或策略变化会形成新的范围指纹。
- 发布规划器要求多目标环境精确选择 `deployment_target_key`，状态按目标隔离，并为外部 Git 组件读取当前设备绑定仓库自己的 commit、分支和工作区状态。
- 共享数据库/服务迁移按 `service_ref` 聚合，每个部署目标的一次发布只安排一个组件执行，避免多端重复迁移。
- 旧单服务器与旧发布状态保持只读兼容；空默认组件不再代表全部组件，新组件也不会自动进入生产范围。
...
- 派生项目模板漂移计划同时比较模板 commit 与 `.maw/seed.lock`，模板基线已对齐但 Seed 版本或契约指纹落后时返回 `seed_contract_behind`，不再误报 `up_to_date`。
- 模块确认方与环境角色、Profile、运行模式、部署模式保留标准推荐选项，同时允许符合小写稳定键约束的项目自定义事实；未知格式继续失败关闭。
- Seed Git、Seed Kit 与 Contract 指纹版本族同步推进到 `v2.3.3`，迁移仍采用预览、隔离校验、人工确认、哈希保护和项目事实保留。

## v2.3.2 - 2026-08-20

### 公开载荷地图资产闭环

- 公开 Seed payload 补齐模块候选、能力目录、项目信号、技术地图、changelog 约定及对应校验/提取脚本，真实公开载荷可直接执行地图 smoke 门。
- 根目录地图索引从 Seed Kit 的公开安全空索引物化，避免复制内部种子仓维护事实，同时让新项目初始化获得同一组空索引基线。
- 新增公开载荷回归测试、升级提示词与 T2 迁移说明，并同步推进 Seed Git、Seed Kit 与 Contract 指纹版本族至 `v2.3.2`。

## v2.3.1 - 2026-08-17

### Seed 三项统一版本族

- 从本版本起，公开 Seed Git tag 与 `mawflow-seed-kit` 使用同一个完整 SemVer，Seed Contract 版本固定由该 SemVer 的主版本推导；重大升级必须三者同时推进。
- 新增统一版本校验，覆盖公开 payload、Python 包、Catalog、BOM、lock、模板元数据和契约指纹，并接入分发、开源与公开产物 smoke 门。
- 历史 `v0.2.x` tag 保持不变；`v2.3.1` 是第一条统一候选线，正式公开发布仍由主仓发布流程和外部回读确认。
...
- 新增当前版本契约安全修复规划器：只对能够确定含义的技术栈旧字段生成白名单 ChangeSet，继续要求人工确认、文件哈希冲突保护、应用后完整编译校验和私有备份。
- 修复旧项目迁移保留无效 `.maw/technology.yaml` 的问题；迁移现在会保留注释与项目字段，只归一化 `build_tool`、框架名称以及服务类型/提供方式，并在预览中列出结构修复。
- 明确版本路由边界：项目 Seed 低于当前 Seed 时进入项目内模板升级，高于 Host 支持版本时先升级 Host；工作台契约修复只服务于版本一致的结构错误。
