...
- 适用范围：把复杂需求、产品方案、实施计划或复盘经验整理为可恢复、可分阶段执行的 AI 任务提示词工程；或在当前会话中执行已有任务提示词工程。已有任务包可以是仓库内目录、本机 zip、远程 zip 直链，或包含下载方式的远程分享页；外部 AI 也可能直接给出可下载、可复制的 Markdown 纯文本，要求接收方 AI 先创建任务包文件再执行。外部 AI 生成的 zip 任务包应优先视为按根目录 `CHATGPT_TO_AI.md` 生成的交接物来校验；`CHATGPT_TO_CODEX.md` 只是兼容指针。
