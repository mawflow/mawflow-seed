...
2. 如果任务给出 `module_key`，先定位它是模块组还是叶子模块；只有叶子模块才读取对应 `doc` 和必要的 `changelog_path`。
...
8. 修改页面 URL、API、数据表、状态流、配置、发布或外部同步边界后，判断是否更新 `docs/modules/<group>/route-api-index.md`、`docs/modules/<module-key>/module.md`、对应 detail docs、`last_verified_commit` 和 `docs/changelogs/<module_key>.md`；执行前遇到旧格式时自动运行迁移脚本。
